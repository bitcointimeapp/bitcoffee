...
function App() {
  ...
  const maxWidth = windowWidth > 900 ? '920px' : '100%'
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        maxWidth: maxWidth,
        margin: '0 auto',
        padding: windowWidth > 600 ? '0 1rem' : '0 0.8rem' 
      }}>
            ...
            {(['de','en','vi'] as const).map(l => (
              <button key={l} onClick={() => setLanguage(l)} 
                style={{ 
                  padding: '6px 14px', 
                  borderRadius: '9999px', 
                  background: language === l ? '#f59e0b' : '#333', 
                  color: language === l ? '#111' : 'white',
                  fontSize: windowWidth < 400 ? '0.9rem' : '1rem'
                }}>
                {l === 'de' && '🇩🇪'} {l === 'en' && '🇬🇧'} {l === 'vi' && '🇻🇳'}
              </button>
            ))}
            ...
            {['home', 'menu', 'reservation', 'bitictionary'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab as Tab)}
                style={{ 
                  ...
                  fontSize: windowWidth < 400 ? '0.85rem' : '0.95rem'
                }}>
                {tab === 'home' && t.home}
                {tab === 'menu' && t.menu}
